...
function phylomon_inner_custom_box() {
    ...
    if($post->ID):
		...
		$diy_terms = get_the_terms( $post->ID, 'diy-classification' );
   		   		   	
   	  	$diy_classification_terms = array();
                if(is_array($diy_terms)):
   	  	foreach ($diy_terms as $diy_term):
   	  		$diy_classification_terms[] = $diy_term->name;
   	  	endforeach;
   	  	endif;
   	  	$diy_classification_in_terms = array_diff  (  $phylomon_diy_classification_order_array ,$diy_classification_terms);
		$diy_classification_in_order = array_diff  (  $diy_classification_terms, $phylomon_diy_classification_order_array );
  ...
  <div class="phylomon_container">
  <h4><label for="phylomon_diy_classification_order"><?php echo __("DIY Classification Order", 'phylomon_textdomain' ) ;?></label></h4>
  
  <p>
   	  <input type="text" id="phylomon_diy_classification_order" name="phylomon_diy_classification_order" value="<?php if(is_array($phylomon_diy_classification_order_array)) { echo implode(", ",$phylomon_diy_classification_order_array); } ?>" size="50" />
   	  <span>(Seperate terms using commas)<br /></span>
   	  <div class="diy_classification">DIY Classification is not ordered yet: <span id="diy_classification_in_order"><?php  if(is_array($diy_classification_in_order) && !empty($diy_classification_in_order)) { echo implode(", ",$diy_classification_in_order );  ?></span> <a href="#" class="add" id="diy_classification_in_order_action">add them to the DIY Classification List</a><?php } ?></div>   	  
   	  <div class="diy_classification">DIY Classification not specified yet: <span id="diy_classification_in_terms"><?php if(is_array($diy_classification_in_terms) && !empty($diy_classification_in_terms)) { echo implode(", ",$diy_classification_in_terms ); ?></span> <a href="#" class="add" id="diy_classification_in_terms_action">add them to the DIY Classification Taxonomy</a><?php } ?></div>
  </p>
  </div>
  ...
  <p>
      ...
      <select id="phylomon_habitat1" name="phylomon_habitat1">    
	      <option <?php if($phylomon_habitat1 == "none") { echo "selected='selected' ";} ?> value="none">None</option> 
  ...
  <p>
          ...
          <script type="text/javascript">
      		jQuery(document).ready(function() {
            	jQuery("#phylomon_card_content").addClass("mceEditor");
             	if ( typeof( tinyMCE ) == "object" && typeof( tinyMCE.execCommand ) == "function" ) {
             		tinyMCE.execCommand("mceAddControl", false, "phylomon_card_content");
				}
...
function phylomon_save_postdata( $post_id ) {
  ...
  if ( 'page' == $_POST['post_type'] ) {
    ...
    update_post_meta($post_id, "_phylomon_card_content",     	    $_POST["phylomon_card_content"]);
	...
	// DIY Classification Order
    $phylomon_diy_classification_order = explode(",", trim( esc_attr( $_POST["phylomon_diy_classification_order"] ) ) );
    
  	$phylomon_diy_classification_order_clean = array();
    
    foreach($phylomon_diy_classification_order as $order):
    	$phylomon_diy_classification_order_clean[] = trim($order);
    endforeach;
    
    update_post_meta($post_id, "_phylomon_diy_classification_order", implode(",",$phylomon_diy_classification_order_clean));
